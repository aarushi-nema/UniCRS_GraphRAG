...
class Dialogue_DBpedia:
    def __init__(self, debug=False):
        self.debug = debug

        self.dataset_dir = '/home/Nema/UniCRS_GraphRAG/Recommendation_GraphRAG'
        with open(os.path.join(self.dataset_dir, 'dialogue_subkg.json'), 'r', encoding='utf-8') as f:
            self.entity_kg = json.load(f)
        with open(os.path.join(self.dataset_dir, 'dialogue_entity2id.json'), 'r', encoding='utf-8') as f:
            self.entity2id = json.load(f)
        with open(os.path.join(self.dataset_dir, 'dialogue_relation2id.json'), 'r', encoding='utf-8') as f:
            self.relation2id = json.load(f)
        with open(os.path.join(self.dataset_dir, 'dialogue_item_ids.json'), 'r', encoding='utf-8') as f:
